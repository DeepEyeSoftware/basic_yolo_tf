...
def YOLOv4(input_layer, NUM_CLASS):
    ...
    route_1 = conv
    conv = common.convolutional(conv, (3, 3, 128, 256))
    conv_sbbox = common.convolutional(conv, (1, 1, 256, 3 * (NUM_CLASS + 5)), activate=False, bn=False, name='fm_0')
    ...
    route_2 = conv
    conv = common.convolutional(conv, (3, 3, 256, 512))
    conv_mbbox = common.convolutional(conv, (1, 1, 512, 3 * (NUM_CLASS + 5)), activate=False, bn=False, name='fm_1')
    ...
    conv = common.convolutional(conv, (3, 3, 512, 1024))
    conv_lbbox = common.convolutional(conv, (1, 1, 1024, 3 * (NUM_CLASS + 5)), activate=False, bn=False, name='fm_2')
...
def decode_train(conv_output, output_size, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):
    ...
    pred_xy = ( (tf.sigmoid(conv_raw_dxdy) * XYSCALE[i]) - 0.5 * (XYSCALE[i] - 1) + xy_grid) * STRIDES[i] # cx, cy input scale
    pred_wh = (tf.exp(conv_raw_dwdh) * ANCHORS[i])                                                        # w, h input scale
    pred_xywh = tf.concat([pred_xy, pred_wh], axis=-1)

    pred_conf = tf.sigmoid(conv_raw_conf)
    pred_prob = tf.sigmoid(conv_raw_prob)

    return tf.keras.layers.concatenate([pred_xywh, pred_conf, pred_prob], axis=-1, name='bbox_{}'.format(i))
...
def compute_loss(pred, conv, label, bboxes, STRIDE, NUM_CLASS, IOU_LOSS_THRESH):
    if (label == None) and (bboxes == None):
        return 0, 0, 0
    ...
    conv_raw_prob = conv[:, :, :, :, 5:]    # pred labels

    pred_xywh     = pred[:, :, :, :, 0:4]   # pred_xywh
    pred_conf     = pred[:, :, :, :, 4:5]   # pred_obj
    
    label_xywh    = label[:, :, :, :, 0:4]  # target_xywh
    respond_bbox  = label[:, :, :, :, 4:5]  # target_obj
    label_prob    = label[:, :, :, :, 5:]   # target labels
    ...
    # Calculate the IOU between each prediction (batch, outSize, outSize, no_anchors, 4) and each target bounding box (batch, 150, 4),
    # which will result in an IOU tensor of shape (batch, outSize, OutSize, no_anchors, 150) 
    iou = utils.bbox_iou(pred_xywh[:, :, :, :, np.newaxis, :], bboxes[:, np.newaxis, np.newaxis, np.newaxis, :, :])
    max_iou = tf.expand_dims(tf.reduce_max(iou, axis=-1), axis=-1) # Max IOU for each box, then expand (batch, outSize, OutSize, no_anchors, 1)

    # Basically objecness when there is no object. Opposite of target objectness (1-respond_bbox), 
    # then sets to 0 where maxIOU is greater than IOU_LOSS_THRESH (if max_iou greater than IOU_LOSS_THRESH -> False -> 0, thus no included in loss)
